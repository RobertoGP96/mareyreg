...
- **Editar y eliminar entregas**: admin y despachador (`assertRole("dispatcher")` en `updateCashDelivery` / `deleteCashDelivery`), en cualquier estado, incluidas las entregadas y canceladas; viewer solo consulta. Editar no cambia el estado ni el eje de la comisión. Marcar entregada y cancelar siguen exigiendo estado pendiente. La UI usa `lib/permissions.ts` (`canManageDeliveries`) solo para ocultar botones.
