...
| 8 | Entregas: multi-monto, denominaciones, mensajero, comisión y foto | ✅ | (movido) |

> **Entregas, destinatarios y mensajeros ya no viven aquí.** Se extrajeron al módulo `entregas` (permiso propio, rutas `/entregas/*`). Ver [docs/ENTREGAS.md](ENTREGAS.md). Este módulo conserva el catálogo `Currency` / `CurrencyDenomination`, que entregas consume.
...
- **Denominaciones**: el catálogo `CurrencyDenomination` se administra desde `/envios/monedas` y DEBE llegar a la unidad mínima de cada moneda; lo consume el módulo entregas para el desglose de billetes.
...
La migración histórica a entregas multi-línea (`envios-delivery-lines-pre/ddl/post.sql`) y los constraints de entregas están documentados en [docs/ENTREGAS.md](ENTREGAS.md).
...
8. **Entregas, destinatarios y mensajeros**: viven en el módulo `entregas` (`/entregas`, `/entregas/destinatarios`, `/entregas/mensajeros`). Las URLs viejas bajo `/envios/*` redirigen.

## Riesgos / pendientes

- **⚠ El catálogo de denominaciones debe llegar a la unidad mínima de cada moneda.** El desglose es obligatorio en entregas y el monto se deriva de él: si CUP solo tuviera 1000/500/200, una entrega de 1 250 CUP sería imposible de registrar. El seed baja hasta 1 (CUP) y 0.01 (USD). USDT es digital y usa cortes sintéticos — si aparecen más monedas digitales, conviene eximirlas del desglose obligatorio.
