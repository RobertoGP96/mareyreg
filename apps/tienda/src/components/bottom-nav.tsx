...
export function BottomNav() {
  ...
  return (
    <>
      {/* La barra es fija, así que taparía el final del documento: el hueco lo
          reserva este espaciador, que solo existe cuando la barra se muestra. */}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-5 border-t border-line bg-canvas md:hidden">
        {ITEMS.map((item) => {
          ...
          return (
            ...
            >
              {/* lucide no publica variante rellena: el activo se marca
                  rellenando el propio trazo y bajando el grosor para que no se
                  empaste. */}
              <Icon
