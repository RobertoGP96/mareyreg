...
> Para profundizar: [docs/AI_WORKFLOW.md](docs/AI_WORKFLOW.md), [docs/MODULE_TEMPLATE.md](docs/MODULE_TEMPLATE.md), [docs/ENVIOS.md](docs/ENVIOS.md), [docs/ENTREGAS.md](docs/ENTREGAS.md).
...
- Comandos por app: `pnpm --filter erp <script>` / `pnpm --filter tienda <script>`. Los comandos de DB (`db:push`, `db:generate`, `db:studio`, `db:sql <archivo>`, `db:entregas`) tienen proxy en la raíz y fijan el cwd en `apps/erp` (dotenv de `prisma.config.ts` carga el `.env` del cwd). El SQL crudo se aplica con `pnpm db:sql prisma/sql/<archivo>.sql` (WebSocket, sin psql). La terminal del usuario es PowerShell 5.1: no encadenar con `&&`, y no usar `pnpm dlx tsx` (se cuelga en el prompt de aprobación de builds de pnpm 10).
...
- **Roles y módulos por rol**: los roles son el enum fijo `UserRole` (no hay roles custom; no añadir "Nuevo rol"). Cada rol concede módulos por defecto en `RoleModulePermission` (`/settings/roles/[role]`, pestañas Permisos/Miembros; admin no se edita, pasa siempre). Acceso efectivo = `UserModulePermission` ∪ módulos del rol, resuelto SOLO en `authorize` de `src/lib/auth.ts` con `mergeEffectiveModules` (`src/modules/auth/lib/effective-modules.ts`); la sesión es JWT, así que un cambio de permisos aplica al siguiente login. Definiciones compartidas (`ROLE_DEFS`, `isSystemRole`) en `src/modules/auth/lib/roles.ts`.
...
- **Webstore modelos/formatos**: un "modelo" (talla, color) es SIEMPRE un `Product` completo (SKU, stock, precio, foto propios); nunca una presentación. La agrupación para la card de la tienda vive en `WebstoreModelGroup` + `Product.modelGroupId/modelLabel/modelSortOrder` y se gestiona solo en `/webstore/catalogo` (`src/modules/webstore/lib/sync-model-group-members.ts` aplica las reglas: sin servicios, misma unidad y mismo `isCatchWeight` en el grupo, SKU obligatorio, etiqueta única). El catálogo `GET /api/webstore/products` sigue plano (una fila por modelo) con campo `modelGroup` y `presentations[].price` efectivo (vía `getEffectiveLinePrices`, mapeo en `lib/catalog-mappers.ts`); la tienda agrupa en cliente (`apps/tienda/src/lib/model-groups.ts`). El "formato" (unidad/caja) es la `ProductPresentation` de siempre. `Product.name` conserva el nombre completo del modelo para POS/facturas. Tras `db push` aplicar `prisma/sql/webstore-model-groups.sql`.
...
- **Entregas de efectivo (módulo `entregas`, extraído de envios)**: permiso propio `entregas`, rutas `/entregas`, `/entregas/[id]`, `/entregas/destinatarios`, `/entregas/proveedores`, `/entregas/mensajeros` (las viejas `/envios/entregas|destinatarios|mensajeros` redirigen). Los modelos conservan su nombre (`CashDelivery`, `Recipient`, `CourierProfile`). El proveedor de una entrega (`DeliveryProvider`, solo nombre + `active`, `CashDelivery.providerId` opcional) es catálogo propio del módulo — NO reutilizar el `Supplier` de compras; se crea inline desde el picker del formulario o en `/entregas/proveedores`. Una `CashDelivery` es multi-línea — N `CashDeliveryLine` (una por moneda, `@@unique([deliveryId, currencyId])`); el encabezado NO tiene `amount`/`currencyId`. El `amount` de la línea es **derivado** de Σ(`unitValue` × `quantity`) del desglose por denominaciones y se resuelve SIEMPRE server-side en `resolveDeliveryLines` (`src/modules/entregas/lib/delivery-lines.ts`) — el cliente nunca envía montos ni valores de billete. El desglose es obligatorio en Zod pero opcional a nivel DB (las entregas migradas no lo tienen). El catálogo `CurrencyDenomination` (sigue en envios, `/envios/monedas`) DEBE incluir la unidad mínima de cada moneda; entregas lo lee vía `queries/catalog-queries.ts` — única dependencia entre ambos módulos. Mensajero = `User` con fila en `CourierProfile`; su comisión/moneda por defecto solo pre-llenan el form. La comisión tiene eje de estado propio (`commissionStatus`) y cancelar la entrega la revierte. **Galería**: `CashDeliveryPhoto` (máx. 10 por entrega, `kind` receipt/id_document/other, nota, `uploadedById`); se escribe solo vía `lib/delivery-photos.ts` (`appendPhotos`/`syncPhotos`), el cliente sube a Blob por `/api/deliveries/upload` y manda URLs; `CashDelivery.photoUrl` está DEPRECADO (no escribir). Tras `db push` correr `pnpm db:entregas` desde la raíz (aplica en orden `entregas-photos-ddl.sql`, `entregas-constraints.sql`, `entregas-photos.sql` con backfill de `photo_url`, `entregas-permissions.sql` y `entregas-providers.sql`); ver [docs/ENTREGAS.md](docs/ENTREGAS.md).
