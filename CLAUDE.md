...
- **Entregas de efectivo (módulo `entregas`, extraído de envios)**: permiso propio `entregas`, rutas `/entregas`, `/entregas/[id]`, `/entregas/destinatarios`, `/entregas/mensajeros` (las viejas `/envios/entregas|destinatarios|mensajeros` redirigen). Los modelos conservan su nombre (`CashDelivery`, `Recipient`, `CourierProfile`). Una `CashDelivery` es multi-línea — N `CashDeliveryLine` (una por moneda, `@@unique([deliveryId, currencyId])`); el encabezado NO tiene `amount`/`currencyId`. El `amount` de la línea es **derivado** de Σ(`unitValue` × `quantity`) del desglose por denominaciones y se resuelve SIEMPRE server-side en `resolveDeliveryLines` (`src/modules/entregas/lib/delivery-lines.ts`) — el cliente nunca envía montos ni valores de billete. El desglose es obligatorio en Zod pero opcional a nivel DB (las entregas migradas no lo tienen). El catálogo `CurrencyDenomination` (sigue en envios, `/envios/monedas`) DEBE incluir la unidad mínima de cada moneda; entregas lo lee vía `queries/catalog-queries.ts` — única dependencia entre ambos módulos. Mensajero = `User` con fila en `CourierProfile`; su comisión/moneda por defecto solo pre-llenan el form. La comisión tiene eje de estado propio (`commissionStatus`) y cancelar la entrega la revierte. **Galería**: `CashDeliveryPhoto` (máx. 10 por entrega, `kind` receipt/id_document/other, nota, `uploadedById`); se escribe solo vía `lib/delivery-photos.ts` (`appendPhotos`/`syncPhotos`), el cliente sube a Blob por `/api/deliveries/upload` y manda URLs; `CashDelivery.photoUrl` está DEPRECADO (no escribir). Tras `db push` correr `pnpm db:entregas` desde la raíz (aplica en orden `entregas-photos-ddl.sql`, `entregas-constraints.sql`, `entregas-photos.sql` con backfill de `photo_url` y `entregas-permissions.sql`); ver [docs/ENTREGAS.md](docs/ENTREGAS.md).
