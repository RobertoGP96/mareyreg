...
- **Entregas de efectivo (envios)**: una `CashDelivery` es multi-línea — N `CashDeliveryLine` (una por moneda, `@@unique([deliveryId, currencyId])`); el encabezado NO tiene `amount`/`currencyId`. El `amount` de la línea es **derivado** de Σ(`unitValue` × `quantity`) del desglose por denominaciones y se resuelve SIEMPRE server-side en `resolveDeliveryLines` (`src/modules/envios/lib/delivery-lines.ts`) — el cliente nunca envía montos ni valores de billete. El desglose es obligatorio en Zod pero opcional a nivel DB (las entregas migradas no lo tienen). El catálogo `CurrencyDenomination` DEBE incluir la unidad mínima de cada moneda o habrá montos imposibles de capturar. Mensajero = `User` con fila en `CourierProfile`; su comisión/moneda por defecto solo pre-llenan el form. La comisión tiene eje de estado propio (`commissionStatus`) y cancelar la entrega la revierte. Tras `db push` aplicar `prisma/sql/envios-cash-delivery.sql` (y, una sola vez, la secuencia `envios-delivery-lines-pre.sql` → `db push` → `-post.sql`; ver [docs/ENVIOS.md](docs/ENVIOS.md)).
